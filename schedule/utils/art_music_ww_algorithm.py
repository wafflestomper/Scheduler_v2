"""
Flexible algorithm for registering students into art, music, and woodworking classes.
...
def register_art_music_ww_courses(grade_level, undo_depth=3):
    """
    Register students to art, music, and woodworking courses for a specific grade level.
    ...
    """
    print(f"DEBUG: Starting art, music, and WW course registration for grade {grade_level}")
    ...
    target_courses = list(art_courses) + list(music_courses) + list(ww_courses)
    ...
    if len(course_sections) < len(target_courses):
        ...
        return {
            ...
            'art_success': 0,
            'art_failure': len(students),
            'music_success': 0,
            'music_failure': len(students),
            'ww_success': 0,
            'ww_failure': len(students)
        }
    ...
    if success:
        ...
        return {
            'status': 'success',
            'message': 'Successfully registered all students to art, music, and WW courses',
            'art_success': len(students),
            'art_failure': 0,
            'music_success': len(students),
            'music_failure': 0,
            'ww_success': len(students),
            'ww_failure': 0
        }
    else:
        ...
        # Get unique students with art course assignments
        art_students = set(Enrollment.objects.filter(
            section__course__in=art_courses,
            student__in=students
        ).values_list('student_id', flat=True))
        
        # Get unique students with music course assignments
        music_students = set(Enrollment.objects.filter(
            section__course__in=music_courses,
            student__in=students
        ).values_list('student_id', flat=True))
        
        # Get unique students with woodworking course assignments
        ww_students = set(Enrollment.objects.filter(
            section__course__in=ww_courses,
            student__in=students
        ).values_list('student_id', flat=True))
        
        art_success = len(art_students)
        music_success = len(music_students)
        ww_success = len(ww_students)
        
        print(f"DEBUG: Art success: {art_success}/{len(students)}")
        print(f"DEBUG: Music success: {music_success}/{len(students)}")
        print(f"DEBUG: WW success: {ww_success}/{len(students)}")
        
        return {
            'status': 'partial',
            'message': message,
            'art_success': art_success,
            'art_failure': len(students) - art_success,
            'music_success': music_success,
            'music_failure': len(students) - music_success,
            'ww_success': ww_success,
            'ww_failure': len(students) - ww_success
        } 